...
describe Week do
  ...
  let(:week_1_tier) {FactoryGirl.create(:week,
                                        settings: [{total_teams:2, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"}],
                                        standings: teams_2.map{|team| team.id},
                                        league: league
  )}

  it "week with 1 tier is valid" do
  ...
  let(:week_2_tiers) {FactoryGirl.create(:week,
                                        settings: [{total_teams:2, teams_down: 1, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"},
                                                   {total_teams:2, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"}],
  ...
  let(:week_3_tiers) {FactoryGirl.create(:week,
                                         settings: [{total_teams:2, teams_down: 1, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"},
                                                    {total_teams:2, teams_down: 1, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"},
                                                    {total_teams:2, schedule_pattern: League::DEFAULT_SCHEDULE_PATTERNS[2].dup, day: "Mon"},
                                         ],
  ...
  it "number of teams in registrations is different from number of teams in schedule pattern" do
